...
describe('Model', function () {
  ...
  describe('Serialization, deserialization', function () {
    ...
    it('Reject field names beginning with a $ sign or containing a dot, except the four edge cases', function () {
      ...
      // Normal cases
      (function () { b = serialize(a1); }).should.throw();
      (function () { b = serialize(a2); }).should.throw();
  ...
  describe('Object checking', function () {
    ...
    it('Field names beginning with a $ sign are forbidden', function () {
      ...
      (function () {
        checkObject({ some: 42, nested: [ 5, "no", "$actuallyok", true, { $hidden: "useless" } ] });
      }).should.throw();
    });
  ...
  describe('Querying', function () {
    ...
    describe('Field equality', function () {
      ...
      it('Can find documents with simple fields', function () {
        matchQuery({ test: 'yeah' }, { test: 'yea' }).should.equal(false);
        matchQuery({ test: 'yeah' }, { test: 'yeahh' }).should.equal(false);
        matchQuery({ test: 'yeah' }, { test: 'yeah' }).should.equal(true);
      });

      it('Can find documents with the dot-notation', function () {
        matchQuery({ test: { ooo: 'yeah' } }, { "test.ooo": 'yea' }).should.equal(false);
        matchQuery({ test: { ooo: 'yeah' } }, { "test.oo": 'yeah' }).should.equal(false);
        matchQuery({ test: { ooo: 'yeah' } }, { "tst.ooo": 'yeah' }).should.equal(false);
        matchQuery({ test: { ooo: 'yeah' } }, { "test.ooo": 'yeah' }).should.equal(true);
      });

      it('Cannot find undefined', function () {
        matchQuery({ test: undefined }, { test: undefined }).should.equal(false);
        matchQuery({ test: { pp: undefined } }, { "test.pp": undefined }).should.equal(false);
      });

      it('Nested objects are deep-equality matched and not treated as sub-queries', function () {
        matchQuery({ a: { b: 5 } }, { a: { b: 5 } }).should.equal(true);
        matchQuery({ a: { b: 5, c: 3 } }, { a: { b: 5 } }).should.equal(false);

        matchQuery({ a: { b: 5 } }, { a: { b: { $lt: 10 } } }).should.equal(false);
        (function () { matchQuery({ a: { b: 5 } }, { a: { $or: [ { b: 10 }, { b: 5 } ] } }) }).should.throw();
      });
      
      it("Can match for field equality inside an array with the dot notation", function () {
        matchQuery({ a: true, b: [ 'node', 'embedded', 'database' ] }, { 'b.1': 'node' }).should.equal(false);
        matchQuery({ a: true, b: [ 'node', 'embedded', 'database' ] }, { 'b.1': 'embedded' }).should.equal(true);
        matchQuery({ a: true, b: [ 'node', 'embedded', 'database' ] }, { 'b.1': 'database' }).should.equal(false);
      })
    ...
    describe('Regular expression matching', function () {
      ...
      it('Matching a non-string to a regular expression always yields false', function () {
        ...
        matchQuery({ test: true }, { test: /true/ }).should.equal(false);
        matchQuery({ test: null }, { test: /null/ }).should.equal(false);
        matchQuery({ test: 42 }, { test: /42/ }).should.equal(false);
        matchQuery({ test: d }, { test: r }).should.equal(false);
      });

      it('Can match strings using basic querying', function () {
        matchQuery({ test: 'true' }, { test: /true/ }).should.equal(true);
        matchQuery({ test: 'babaaaar' }, { test: /aba+r/ }).should.equal(true);
        matchQuery({ test: 'babaaaar' }, { test: /^aba+r/ }).should.equal(false);
        matchQuery({ test: 'true' }, { test: /t[ru]e/ }).should.equal(false);
      });

      it('Can match strings using the $regex operator', function () {
        matchQuery({ test: 'true' }, { test: { $regex: /true/ } }).should.equal(true);
        matchQuery({ test: 'babaaaar' }, { test: { $regex: /aba+r/ } }).should.equal(true);
        matchQuery({ test: 'babaaaar' }, { test: { $regex: /^aba+r/ } }).should.equal(false);
        matchQuery({ test: 'true' }, { test: { $regex: /t[ru]e/ } }).should.equal(false);
      });

      it('Will throw if $regex operator is used with a non regex value', function () {
        (function () {
          matchQuery({ test: 'true' }, { test: { $regex: 42 } })
        }).should.throw();

        (function () {
          matchQuery({ test: 'true' }, { test: { $regex: 'true' } })
        }).should.throw();
      });

      it('Can use the $regex operator in cunjunction with other operators', function () {
        matchQuery({ test: 'helLo' }, { test: { $regex: /ll/i, $nin: ['helL', 'helLop'] } }).should.equal(true);
        matchQuery({ test: 'helLo' }, { test: { $regex: /ll/i, $nin: ['helLo', 'helLop'] } }).should.equal(false);
      });

      it('Can use dot-notation', function () {
        matchQuery({ test: { nested: 'true' } }, { 'test.nested': /true/ }).should.equal(true);
        matchQuery({ test: { nested: 'babaaaar' } }, { 'test.nested': /^aba+r/ }).should.equal(false);

        matchQuery({ test: { nested: 'true' } }, { 'test.nested': { $regex: /true/ } }).should.equal(true);
        matchQuery({ test: { nested: 'babaaaar' } }, { 'test.nested': { $regex: /^aba+r/ } }).should.equal(false);
      });
    ...
    describe('$lt', function () {
      ...
      it('Cannot compare a field to an object, an array, null or a boolean, it will return false', function () {
        matchQuery({ a: 5 }, { a: { $lt: { a: 6 } } }).should.equal(false);
        matchQuery({ a: 5 }, { a: { $lt: [6, 7] } }).should.equal(false);
        matchQuery({ a: 5 }, { a: { $lt: null } }).should.equal(false);
        matchQuery({ a: 5 }, { a: { $lt: true } }).should.equal(false);
      });

      it('Can compare numbers, with or without dot notation', function () {
        matchQuery({ a: 5 }, { a: { $lt: 6 } }).should.equal(true);
        matchQuery({ a: 5 }, { a: { $lt: 5 } }).should.equal(false);
        matchQuery({ a: 5 }, { a: { $lt: 4 } }).should.equal(false);

        matchQuery({ a: { b: 5 } }, { "a.b": { $lt: 6 } }).should.equal(true);
        matchQuery({ a: { b: 5 } }, { "a.b": { $lt: 3 } }).should.equal(false);
      });

      it('Can compare strings, with or without dot notation', function () {
        matchQuery({ a: "nedb" }, { a: { $lt: "nedc" } }).should.equal(true);
        matchQuery({ a: "nedb" }, { a: { $lt: "neda" } }).should.equal(false);

        matchQuery({ a: { b: "nedb" } }, { "a.b": { $lt: "nedc" } }).should.equal(true);
        matchQuery({ a: { b: "nedb" } }, { "a.b": { $lt: "neda" } }).should.equal(false);
      });

      it('If field is an array field, a match means a match on at least one element', function () {
        matchQuery({ a: [5, 10] }, { a: { $lt: 4 } }).should.equal(false);
        matchQuery({ a: [5, 10] }, { a: { $lt: 6 } }).should.equal(true);
        matchQuery({ a: [5, 10] }, { a: { $lt: 11 } }).should.equal(true);
      });

      it('Works with dates too', function () {
        matchQuery({ a: new Date(1000) }, { a: { $gte: new Date(1001) } }).should.equal(false);
        matchQuery({ a: new Date(1000) }, { a: { $lt: new Date(1001) } }).should.equal(true);
      });
    ...
    describe('Other comparison operators: $lte, $gt, $gte, $ne, $in, $exists', function () {
      ...
      it('$lte', function () {
        matchQuery({ a: 5 }, { a: { $lte: 6 } }).should.equal(true);
        matchQuery({ a: 5 }, { a: { $lte: 5 } }).should.equal(true);
        matchQuery({ a: 5 }, { a: { $lte: 4 } }).should.equal(false);
      });

      it('$gt', function () {
        matchQuery({ a: 5 }, { a: { $gt: 6 } }).should.equal(false);
        matchQuery({ a: 5 }, { a: { $gt: 5 } }).should.equal(false);
        matchQuery({ a: 5 }, { a: { $gt: 4 } }).should.equal(true);
      });

      it('$gte', function () {
        matchQuery({ a: 5 }, { a: { $gte: 6 } }).should.equal(false);
        matchQuery({ a: 5 }, { a: { $gte: 5 } }).should.equal(true);
        matchQuery({ a: 5 }, { a: { $gte: 4 } }).should.equal(true);
      });

      it('$ne', function () {
        matchQuery({ a: 5 }, { a: { $ne: 4 } }).should.equal(true);
        matchQuery({ a: 5 }, { a: { $ne: 5 } }).should.equal(false);
        matchQuery({ a: 5 }, { b: { $ne: 5 } }).should.equal(true);
        matchQuery({ a: false }, { a: { $ne: false } }).should.equal(false);
      });

      it('$in', function () {
        matchQuery({ a: 5 }, { a: { $in: [6, 8, 9] } }).should.equal(false);
        matchQuery({ a: 6 }, { a: { $in: [6, 8, 9] } }).should.equal(true);
        matchQuery({ a: 7 }, { a: { $in: [6, 8, 9] } }).should.equal(false);
        matchQuery({ a: 8 }, { a: { $in: [6, 8, 9] } }).should.equal(true);
        matchQuery({ a: 9 }, { a: { $in: [6, 8, 9] } }).should.equal(true);

        (function () { matchQuery({ a: 5 }, { a: { $in: 5 } }); }).should.throw();
      });

      it('$nin', function () {
        matchQuery({ a: 5 }, { a: { $nin: [6, 8, 9] } }).should.equal(true);
        matchQuery({ a: 6 }, { a: { $nin: [6, 8, 9] } }).should.equal(false);
        matchQuery({ a: 7 }, { a: { $nin: [6, 8, 9] } }).should.equal(true);
        matchQuery({ a: 8 }, { a: { $nin: [6, 8, 9] } }).should.equal(false);
        matchQuery({ a: 9 }, { a: { $nin: [6, 8, 9] } }).should.equal(false);

        // Matches if field doesn't exist
        matchQuery({ a: 9 }, { b: { $nin: [6, 8, 9] } }).should.equal(true);

        (function () { matchQuery({ a: 5 }, { a: { $in: 5 } }); }).should.throw();
      });

      it('$exists', function () {
        matchQuery({ a: 5 }, { a: { $exists: 1 } }).should.equal(true);
        matchQuery({ a: 5 }, { a: { $exists: true } }).should.equal(true);
        matchQuery({ a: 5 }, { a: { $exists: new Date() } }).should.equal(true);
        matchQuery({ a: 5 }, { a: { $exists: '' } }).should.equal(true);
        matchQuery({ a: 5 }, { a: { $exists: [] } }).should.equal(true);
        matchQuery({ a: 5 }, { a: { $exists: {} } }).should.equal(true);

        matchQuery({ a: 5 }, { a: { $exists: 0 } }).should.equal(false);
    ...
    describe('Comparing on arrays', function () {
      ...
      it("Can perform a direct array match", function () {
        matchQuery({ planets: ['Earth', 'Mars', 'Pluto'], something: 'else' }, { planets: ['Earth', 'Mars'] }).should.equal(false);
        matchQuery({ planets: ['Earth', 'Mars', 'Pluto'], something: 'else' }, { planets: ['Earth', 'Mars', 'Pluto'] }).should.equal(true);
        matchQuery({ planets: ['Earth', 'Mars', 'Pluto'], something: 'else' }, { planets: ['Earth', 'Pluto', 'Mars'] }).should.equal(false);
      });

      it('Can query on the size of an array field', function () {
        // Non nested documents
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens": { $size: 0 } }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens": { $size: 1 } }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens": { $size: 2 } }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens": { $size: 3 } }).should.equal(true);

        // Nested documents
        matchQuery({ hello: 'world', description: { satellites: ['Moon', 'Hubble'], diameter: 6300 } }, { "description.satellites": { $size: 0 } }).should.equal(false);
        matchQuery({ hello: 'world', description: { satellites: ['Moon', 'Hubble'], diameter: 6300 } }, { "description.satellites": { $size: 1 } }).should.equal(false);
        matchQuery({ hello: 'world', description: { satellites: ['Moon', 'Hubble'], diameter: 6300 } }, { "description.satellites": { $size: 2 } }).should.equal(true);
        matchQuery({ hello: 'world', description: { satellites: ['Moon', 'Hubble'], diameter: 6300 } }, { "description.satellites": { $size: 3 } }).should.equal(false);

        // Using a projected array
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.names": { $size: 0 } }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.names": { $size: 1 } }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.names": { $size: 2 } }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.names": { $size: 3 } }).should.equal(true);
      });

      it('$size operator works with empty arrays', function () {
        matchQuery({ childrens: [] }, { "childrens": { $size: 0 } }).should.equal(true);
        matchQuery({ childrens: [] }, { "childrens": { $size: 2 } }).should.equal(false);
        matchQuery({ childrens: [] }, { "childrens": { $size: 3 } }).should.equal(false);
      });

      it('Should throw an error if a query operator is used without comparing to an integer', function () {
        (function () { matchQuery({ a: [1, 5] }, { a: { $size: 1.4 } }); }).should.throw();
        (function () { matchQuery({ a: [1, 5] }, { a: { $size: 'fdf' } }); }).should.throw();
        (function () { matchQuery({ a: [1, 5] }, { a: { $size: { $lt: 5 } } }); }).should.throw();
      });

      it('Using $size operator on a non-array field should prevent match but not throw', function () {
        matchQuery({ a: 5 }, { a: { $size: 1 } }).should.equal(false);
      });

      it('Can use $size several times in the same matcher', function () {
        matchQuery({ childrens: [ 'Riri', 'Fifi', 'Loulou' ] }, { "childrens": { $size: 3, $size: 3 } }).should.equal(true);
        matchQuery({ childrens: [ 'Riri', 'Fifi', 'Loulou' ] }, { "childrens": { $size: 3, $size: 4 } }).should.equal(false);   // Of course this can never be true
      });

      it('Can query array documents with multiple simultaneous conditions', function () {
        // Non nested documents
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens": { $elemMatch: { name: "Dewey", age: 7 } } }).should.equal(true);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens": { $elemMatch: { name: "Dewey", age: 12 } } }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens": { $elemMatch: { name: "Louie", age: 3 } } }).should.equal(false);

        // Nested documents
        matchQuery({ outer: { childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] } }, { "outer.childrens": { $elemMatch: { name: "Dewey", age: 7 } } }).should.equal(true);
        matchQuery({ outer: { childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] } }, { "outer.childrens": { $elemMatch: { name: "Dewey", age: 12 } } }).should.equal(false);
        matchQuery({ outer: { childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] } }, { "outer.childrens": { $elemMatch: { name: "Louie", age: 3 } } }).should.equal(false);

      });

      it('$elemMatch operator works with empty arrays', function () {
        matchQuery({ childrens: [] }, { "childrens": { $elemMatch: { name: "Mitsos" } } }).should.equal(false);
        matchQuery({ childrens: [] }, { "childrens": { $elemMatch: {} } }).should.equal(false);
      });

      it('Can use more complex comparisons inside nested query documents', function () {
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens": { $elemMatch: { name: "Dewey", age: { $gt: 6, $lt: 8 } } } }).should.equal(true);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens": { $elemMatch: { name: "Dewey", age: { $in: [ 6, 7, 8 ] } } } } ).should.equal(true);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens": { $elemMatch: { name: "Dewey", age: { $gt: 6, $lt: 7 } } } }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens": { $elemMatch: { name: "Louie", age: { $gt: 6, $lte: 7 } } } }).should.equal(false);
      });
    });
    ...
    describe('Logical operators $or, $and, $not', function () {
      ...
      it('Any of the subqueries should match for an $or to match', function () {
        matchQuery({ hello: 'world' }, { $or: [ { hello: 'pluton' }, { hello: 'world' } ] }).should.equal(true);
        matchQuery({ hello: 'pluton' }, { $or: [ { hello: 'pluton' }, { hello: 'world' } ] }).should.equal(true);
        matchQuery({ hello: 'nope' }, { $or: [ { hello: 'pluton' }, { hello: 'world' } ] }).should.equal(false);
        matchQuery({ hello: 'world', age: 15 }, { $or: [ { hello: 'pluton' }, { age: { $lt: 20 } } ] }).should.equal(true);
        matchQuery({ hello: 'world', age: 15 }, { $or: [ { hello: 'pluton' }, { age: { $lt: 10 } } ] }).should.equal(false);
      });

      it('All of the subqueries should match for an $and to match', function () {
        matchQuery({ hello: 'world', age: 15 }, { $and: [ { age: 15 }, { hello: 'world' } ] }).should.equal(true);
        matchQuery({ hello: 'world', age: 15 }, { $and: [ { age: 16 }, { hello: 'world' } ] }).should.equal(false);
        matchQuery({ hello: 'world', age: 15 }, { $and: [ { hello: 'world' }, { age: { $lt: 20 } } ] }).should.equal(true);
        matchQuery({ hello: 'world', age: 15 }, { $and: [ { hello: 'pluton' }, { age: { $lt: 20 } } ] }).should.equal(false);
      });

      it('Subquery should not match for a $not to match', function () {
        matchQuery({ a: 5, b: 10 }, { a: 5 }).should.equal(true);
        matchQuery({ a: 5, b: 10 }, { $not: { a: 5 } }).should.equal(false);
      });

      it('Logical operators are all top-level, only other logical operators can be above', function () {
        (function () { matchQuery({ a: { b: 7 } }, { a: { $or: [ { b: 5 }, { b: 7 } ] } })}).should.throw();
        matchQuery({ a: { b: 7 } }, { $or: [ { "a.b": 5 }, { "a.b": 7 } ] }).should.equal(true);
      });

      it('Logical operators can be combined as long as they are on top of the decision tree', function () {
        matchQuery({ a: 5, b: 7, c: 12 }, { $or: [ { $and: [ { a: 5 }, { b: 8 } ] }, { $and: [{ a: 5 }, { c : { $lt: 40 } }] } ] }).should.equal(true);
        matchQuery({ a: 5, b: 7, c: 12 }, { $or: [ { $and: [ { a: 5 }, { b: 8 } ] }, { $and: [{ a: 5 }, { c : { $lt: 10 } }] } ] }).should.equal(false);
      });
      ...
      it('Should throw an error if a logical operator is used without an array or if an unknown logical operator is used', function () {
        (function () { matchQuery({ a: 5 }, { $or: { a: 5, a: 6 } }); }).should.throw();
        (function () { matchQuery({ a: 5 }, { $and: { a: 5, a: 6 } }); }).should.throw();
        (function () { matchQuery({ a: 5 }, { $unknown: [ { a: 5 } ] }); }).should.throw();
      });
    ...
    describe('Comparison operator $where', function () {
      ...
      it('Function should match and not match correctly', function () {
        matchQuery({ a: 4}, { $where: function () { return this.a === 4; } }).should.equal(true);
        matchQuery({ a: 4}, { $where: function () { return this.a === 5; } }).should.equal(false);
      });

      it('Should throw an error if the $where function is not, in fact, a function', function () {
        (function () { matchQuery({ a: 4 }, { $where: 'not a function' }); }).should.throw();
      });

      it('Should throw an error if the $where function returns a non-boolean', function () {
        (function () { matchQuery({ a: 4 }, { $where: function () { return 'not a boolean'; } }); }).should.throw();
      });
      ...
      it('Should be able to do the complex matching it must be used for', function () {
        ...
        matchQuery({ firstName: "John", lastName: "Doe", email: "john.doe@gmail.com" }, { $where: checkEmail }).should.equal(true);
        matchQuery({ firstName: "john", lastName: "doe", email: "john.doe@gmail.com" }, { $where: checkEmail }).should.equal(true);
        matchQuery({ firstName: "Jane", lastName: "Doe", email: "john.doe@gmail.com" }, { $where: checkEmail }).should.equal(false);
        matchQuery({ firstName: "John", lastName: "Deere", email: "john.doe@gmail.com" }, { $where: checkEmail }).should.equal(false);
        matchQuery({ lastName: "Doe", email: "john.doe@gmail.com" }, { $where: checkEmail }).should.equal(false);
      });
    ...
    describe('Array fields', function () {
      ...
      it('Field equality', function () {
        matchQuery({ tags: ['node', 'js', 'db'] }, { tags: 'python' }).should.equal(false);
        matchQuery({ tags: ['node', 'js', 'db'] }, { tagss: 'js' }).should.equal(false);
        matchQuery({ tags: ['node', 'js', 'db'] }, { tags: 'js' }).should.equal(true);
        matchQuery({ tags: ['node', 'js', 'db'] }, { tags: 'js', tags: 'node' }).should.equal(true);

        // Mixed matching with array and non array
        matchQuery({ tags: ['node', 'js', 'db'], nedb: true }, { tags: 'js', nedb: true }).should.equal(true);

        // Nested matching
        matchQuery({ number: 5, data: { tags: ['node', 'js', 'db'] } }, { "data.tags": 'js' }).should.equal(true);
        matchQuery({ number: 5, data: { tags: ['node', 'js', 'db'] } }, { "data.tags": 'j' }).should.equal(false);
      });

      it('With one comparison operator', function () {
        matchQuery({ ages: [3, 7, 12] }, { ages: { $lt: 2 } }).should.equal(false);
        matchQuery({ ages: [3, 7, 12] }, { ages: { $lt: 3 } }).should.equal(false);
        matchQuery({ ages: [3, 7, 12] }, { ages: { $lt: 4 } }).should.equal(true);
        matchQuery({ ages: [3, 7, 12] }, { ages: { $lt: 8 } }).should.equal(true);
        matchQuery({ ages: [3, 7, 12] }, { ages: { $lt: 13 } }).should.equal(true);
      });

      it('Works with arrays that are in subdocuments', function () {
        matchQuery({ children: { ages: [3, 7, 12] } }, { "children.ages": { $lt: 2 } }).should.equal(false);
        matchQuery({ children: { ages: [3, 7, 12] } }, { "children.ages": { $lt: 3 } }).should.equal(false);
        matchQuery({ children: { ages: [3, 7, 12] } }, { "children.ages": { $lt: 4 } }).should.equal(true);
        matchQuery({ children: { ages: [3, 7, 12] } }, { "children.ages": { $lt: 8 } }).should.equal(true);
        matchQuery({ children: { ages: [3, 7, 12] } }, { "children.ages": { $lt: 13 } }).should.equal(true);
      });

      it('Can query inside arrays thanks to dot notation', function () {
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.age": { $lt: 2 } }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.age": { $lt: 3 } }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.age": { $lt: 4 } }).should.equal(true);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.age": { $lt: 8 } }).should.equal(true);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.age": { $lt: 13 } }).should.equal(true);
        
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.name": 'Louis' }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.name": 'Louie' }).should.equal(true);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.name": 'Lewi' }).should.equal(false);
      });
      
      it('Can query for a specific element inside arrays thanks to dot notation', function () {
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.0.name": 'Louie' }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.1.name": 'Louie' }).should.equal(false);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.2.name": 'Louie' }).should.equal(true);
        matchQuery({ childrens: [ { name: "Huey", age: 3 }, { name: "Dewey", age: 7 }, { name: "Louie", age: 12 } ] }, { "childrens.3.name": 'Louie' }).should.equal(false);
      });
      
      it('A single array-specific operator and the query is treated as array specific', function () {
        (function () { matchQuery({ childrens: [ 'Riri', 'Fifi', 'Loulou' ] }, { "childrens": { "Fifi": true, $size: 3 } })}).should.throw();
      });
      
      it('Can mix queries on array fields and non array filds with array specific operators', function () {
        matchQuery({ uncle: 'Donald', nephews: [ 'Riri', 'Fifi', 'Loulou' ] }, { nephews: { $size: 2 }, uncle: 'Donald' }).should.equal(false);
        matchQuery({ uncle: 'Donald', nephews: [ 'Riri', 'Fifi', 'Loulou' ] }, { nephews: { $size: 3 }, uncle: 'Donald' }).should.equal(true);
        matchQuery({ uncle: 'Donald', nephews: [ 'Riri', 'Fifi', 'Loulou' ] }, { nephews: { $size: 4 }, uncle: 'Donald' }).should.equal(false);

        matchQuery({ uncle: 'Donals', nephews: [ 'Riri', 'Fifi', 'Loulou' ] }, { nephews: { $size: 3 }, uncle: 'Picsou' }).should.equal(false);
        matchQuery({ uncle: 'Donald', nephews: [ 'Riri', 'Fifi', 'Loulou' ] }, { nephews: { $size: 3 }, uncle: 'Donald' }).should.equal(true);
        matchQuery({ uncle: 'Donald', nephews: [ 'Riri', 'Fifi', 'Loulou' ] }, { nephews: { $size: 3 }, uncle: 'Daisy' }).should.equal(false);
      });
