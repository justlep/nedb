 ...
 * @param {String} [opts.filename] Optional, datastore will be in-memory only if not provided
 * @param {Boolean} [opts.timestampData] Optional, defaults to false. If set to true, createdAt and updatedAt will be created and populated automatically (if not specified by user)
 * @param {Boolean} [opts.inMemoryOnly] Optional, defaults to false
 * @param {String} [opts.nodeWebkitAppName] Optional, specify the name of your NW app if you want opts.filename to be relative to the directory where
 *                                            Node Webkit stores application data such as cookies and local storage (the best place to store data in my opinion)
 * @param {Boolean} [opts.autoload] Optional, defaults to false
 * @param {Function} [opts.onload] Optional, if autoload is used this will be called after the load database with the error object as parameter. If you don't pass it the error will be thrown
 * @param {Function} [opts.afterSerialization/opts.beforeDeserialization] Optional, serialization hooks
 * @param {Number} [opts.corruptAlertThreshold] Optional, threshold after which an alert is thrown if too much data is corrupt
 * @param {Function} [opts.compareStrings] Optional, string comparison function that overrides default for sorting
 *
 * Event Emitter - Events
 * * compaction.done - Fired whenever a compaction operation was finished
 */
export function Datastore(opts = {}) {
    let filename;
    ...
    this.compareStrings = opts.compareStrings;

    // Persistence handling
    this.persistence = new Persistence({
        db: this, nodeWebkitAppName: opts.nodeWebkitAppName
    ...
    if (this.autoload) {
        this.loadDatabase(opts.onload || function (err) {
            if (err) {
                throw err;
            }
        });
...
Datastore.prototype.getCandidates = function (query, dontExpireStaleDocs, callback) {
    ...
    async.waterfall([
        ...
        function (cb) {
            ...
            // For a $in match
            usableQueryKeys = [];
            Object.keys(query).forEach(k => hasOwnProp(query[k], '$in') && usableQueryKeys.push(k));
            ...
            usableQueryKeys = [];
            Object.keys(query).forEach(k => hasAnyOwnPropOf(query[k], '$lt', '$lte', '$gt', '$gte') && usableQueryKeys.push(k));
        ...
        , function (docs) {
            ...
            let expiredDocsIds = [], validDocs = [], ttlIndexesFieldNames = Object.keys(self.ttlIndexes);
...
Datastore.prototype.createNewId = function () {
    let tentativeId = generateUid(16);
