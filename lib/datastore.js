...
export class Datastore extends EventEmitter {
     ...
     * @param {String} [nodeWebkitAppName] Optional, specify the name of your NW app if you want filename to be relative to the directory where
     ...
     */
    constructor({filename, timestampData, inMemoryOnly, nodeWebkitAppName, autoload, onload, beforeDeserialization, afterSerialization, 
    ...
    getCandidates(query, dontExpireStaleDocs, callback) {
        ...
        async.waterfall([
            ...
            (docs) => {
                ...
                async.eachSeries(expiredDocsIds, (_id, cb) => this._remove({_id}, {}, err => err ? callback(err) : cb()),
        ...
        async.waterfall([
            (cb) => {
                ...
                cursor.limit(1)._exec((err, docs) => {
                    ...
                    return this._insert(toBeInserted, (err, newDoc) => err ? callback(err) : callback(null, 1, newDoc, true));
                });
            },
            () => {   // Perform the update
                let modifications = [],
                ...
                this.getCandidates(query, (err, candidates) => {
                    ...
                    try {
                        let dateToUse = this.timestampData ? new Date() : null;
                        for (const oldDoc of candidates) {
                            if (model.match(oldDoc, query) && (multi || !totalReplaced)) {
                                totalReplaced++;
                                
                                let oldCreatedAt = oldDoc.createdAt,
                                    newDoc = model.modify(oldDoc, updateQuery);
                                
                                if (dateToUse) {
                                    newDoc.createdAt = oldCreatedAt;
                                    newDoc.updatedAt = dateToUse;
                                }
