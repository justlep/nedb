...
import {checkObject, cloneDeep, matchQuery} from './model.js';
...
export class Datastore extends EventEmitter {
    ...
    _insert(docOrDocs, cb) {
        ...
        try {
            preparedDocs = this.prepareDocumentForInsertion(docOrDocs);
            if (Array.isArray(preparedDocs)) {
                this._addMultipleDocsToIndexes(preparedDocs);
            } else {
                singleDoc = preparedDocs;
                this.addToIndexes(singleDoc);
                preparedDocs = [singleDoc];
            }
        } catch (err) {
            return cb?.(err);
        }

        this.persistence.persistNewState(preparedDocs, err => cb?.(err || null, err ? null : cloneDeep(singleDoc || preparedDocs)));
    }
     ...
     * @api private
     */
    prepareDocumentForInsertion(docOrDocs) {
        if (Array.isArray(docOrDocs)) {
            return docOrDocs.map(doc => this.prepareDocumentForInsertion(doc));
        } 
        
        let preparedDoc = cloneDeep(docOrDocs);
    ...
    find(query, projection, callback) {
        ...
        let cursor = new Cursor(this, query, (err, docs, cb) => err ? cb(err) : cb(null, docs.map(doc => cloneDeep(doc))), projection);
    ...
    findOne(query, projection, callback) {
        ...
        let cursor = new Cursor(this, query, 
                                (err, docs, cb) => err ? cb(err) : cb(null, docs.length === 1 ? cloneDeep(docs[0]) : null), projection, 1);
    ...
    _updateOrInsert(query, updateQuery, options, callback = NOP) {
        ...
        new Cursor(this, query).limit(1)._exec((err, docs) => {
            ...
            try {
                ...
                try {
                    toBeInserted = model.modify(cloneDeep(query, true), updateQuery);
                } catch (err) {
                    return callback(err);
                }
            }
