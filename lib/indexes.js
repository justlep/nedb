...
import {isDate} from './customUtils.js';
...
function projectForUnique(elt) {
    ...
    switch (typeof elt) {
        case 'string': return '$string' + elt;
        case 'boolean': return '$boolean' + elt;
        case 'number': return '$number' + elt;
    }
    // Arrays and objects, will check for pointer equality
    return isDate(elt) ? ('$date' + elt.getTime()) : elt;
}
...
export class Index {
    ...
    insert(doc) {
        ...
        let keys = _.uniq(key, projectForUnique),
            totalRevertible = 0;
        ...
        try {
            ...
            while (totalRevertible) {
                this.tree.delete(keys[--totalRevertible], doc);
            }
            throw err;
        }
    }
    ...
    remove(docOrDocs) {
        ...
        let key = getDotValue(docOrDocs, this.fieldName);
        if (key === undefined && this.sparse) {
            return;
        }

        if (Array.isArray(key)) {
            for (const _k of _.uniq(key, projectForUnique)) {
                this.tree.delete(_k, docOrDocs);
            }
        } else {
            this.tree.delete(key, docOrDocs);
        }
    }
