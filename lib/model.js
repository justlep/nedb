...
export function compareThings(a, b, _compareStrings) {
    ...
    // Strings
    if (typeof a === 'string') {
        return typeof b === 'string' ? (_compareStrings??compareNSB)(a, b) : -1;
    }
    if (typeof b === 'string') {
        return 1;
    }
 ...
 */
export function modifyDoc(obj, updateQuery) {
    let keys = Object.keys(updateQuery),
        totalDollarKeys = keys.reduce((n, key) => key[0] === '$' ? n+1 : n, 0),
        newDoc;

    if (~keys.indexOf('_id') && updateQuery._id !== obj._id) {
