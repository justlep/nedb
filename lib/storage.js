...
export function crashSafeWriteFile(filename, data, callback) {
    ...
    async.waterfall([
        ...
        (cb) => fileExists(filename, (exists) => exists ? flushToStorage(filename, err => cb(err)) : cb()),
        (cb) => __writeFile(tempFilename, data, (err) => cb(err)),
        async.apply(flushToStorage, tempFilename),
        (cb) => rename(tempFilename, filename, (err) => cb(err)),
        async.apply(flushToStorage, {filename: dirname(filename), isDir: true})
    ], (err) => callback?.(err));
}
