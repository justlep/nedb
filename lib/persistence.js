...
export class Persistence {
    ...
    loadDatabase(cb) {
        ...
        async.waterfall([
            function (cb) {
                Persistence.ensureDirectoryExists(dirname(self.filename), function (err) {
                    ensureDatafileIntegrity(self.filename, function (err) {
                        readFile(self.filename, 'utf8', function (err, rawData) {
                            if (err) {
                                return cb(err);
                            }
                            let treatedData;
