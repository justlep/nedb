...
export class Persistence {
    ...
    /**
     * Stop auto-compaction (do nothing if auto-compaction was not running)
     */
    stopAutocompaction() {
    ...
    treatRawData(rawData) {
        // TODO consider switching to Node's readline interface here
    ...
    loadDatabase(cb) {
        ...
        // this used to be wrapped in one single task in an async.waterfall in original NeDB which used async@0.2.10. 
        // waterfall in async@0.2.10 would start the jobs via 'setImmediate',  but async@3 does NOT.
        // we need 'setImmediate' here in order to remain crash-safe
        setImmediate(() => ensureDirectoryExists(dirname(this.filename), (err) => {
            if (err) {
                return __safeCallback(err);
            }
            ensureDatafileIntegrity(this.filename, (err) => err ? __safeCallback(err) : readFile(this.filename, 'utf8', (err, rawData) => {
                if (err) {
                    return __safeCallback(err);
                }
                let treatedData;
                ...
                this.db.persistence.persistCachedDatabase(__safeCallback);
            }));
        }));
    }
}
