...
export class Persistence {
     ...
     */
    constructor({db, corruptAlertThreshold = 0.1, afterSerialization, beforeDeserialization}) {
        this.db = db;
    ...
    persistCachedDatabase(cb = NOP) {
        ...
        for (const fieldName of Object.keys(this.db.indexes)) {
            if (fieldName !== '_id') {   // The special _id index is managed by datastore.js, the others need to be persisted
                ...
                toPersist += this.afterSerialization(model.serialize({$$indexCreated: {fieldName, unique, sparse}})) + '\n';
            }
        }
    ...
    treatRawData(rawData) {
        ...
        for (const line of lines) {
            try {
                let doc = model.deserialize(this.beforeDeserialization(line));
                if (doc._id) {
                    if (doc.$$deleted === true) {
